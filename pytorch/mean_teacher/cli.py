...
def create_parser():
    ...
    parser.add_argument('--word-noise', default='drop:1', type=str,
                        help='What type of noise should be added to the input (NLP) and how much; format= [(drop|replace|add|no-noise):X], where replace=replace a random word with a wordnet synonym, drop=random word dropout, X=number of words (default: drop:1) ')
